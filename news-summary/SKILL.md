...
Step 3で抽出した影響度「高」「中」の**全関連銘柄**について、銘柄ノートを自動作成/更新する。
**1銘柄も漏らさない。** ニュース本文に銘柄名+証券コードが登場した銘柄は全件対象。
目安として1回の実行で最低5件、通常10-15件の銘柄ノートが作成/更新される。
3件以下の場合はStep 3の銘柄抽出が不足している可能性があるため、ニュースを再確認する。

### 8-A: 新規作成（ファイルが存在しない場合）

Globツールで `{VAULT}/Stocks/{code}-*.md` を検索し、ファイルがなければWriteツールで作成する。

**銘柄情報の取得**: 新規作成時、WebSearchで `"{銘柄名} {code} 株価 PER PBR 配当利回り site:kabutan.jp"` を検索し、
基本的な株価指標を取得する。取得できない場合は「要確認」と記載するが、**全項目を空欄のままにしない**。
...
{このニュースがこの銘柄にとってなぜ重要か、投資判断にどう影響するかを2-3行で記載。
