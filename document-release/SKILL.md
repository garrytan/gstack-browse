...
- Bump VERSION without asking — always use ask the user directly for version changes
...
For each risky or questionable update identified in Step 2, use ask the user directly with:
...
3. Never regenerate a CHANGELOG entry from scratch. The entry was written by `/ship` from the
   ...
   rewriting history.
4. If an entry looks wrong or incomplete, use ask the user directly — do NOT silently fix it.
...
- Auto-fix minor voice adjustments. Use ask the user directly if a rewrite would alter meaning.
...
5. Flag any contradictions between documents. Auto-fix clear factual inconsistencies (e.g., a
   version mismatch). Use ask the user directly for narrative contradictions.
...
2. **Items needing description updates:** If a TODO references files or components that were
   significantly changed, its description may be stale. Use ask the user directly to confirm whether
   the TODO should be updated, completed, or left as-is.

3. **New deferred work:** Check the diff for `TODO`, `FIXME`, `HACK`, and `XXX` comments. For
   each one that represents meaningful deferred work (not a trivial inline note), use
   ask the user directly to ask whether it should be captured in TODOS.md.
...
3. **If VERSION was NOT bumped:** Ask the user directly:
...
4. **If VERSION was already bumped:** Do NOT skip silently. Instead, check whether the bump
   ...
   d. **If there are significant uncovered changes:** Use ask the user directly explaining what the
