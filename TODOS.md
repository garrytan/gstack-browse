...
**Context:** Google shipped Chrome DevTools MCP in Chrome 146+ (June 2025). It provides screenshots, console messages, performance traces, Lighthouse audits, and full page interaction through the user's real browser. nstack should use it for real-session access while keeping Playwright for headless CI/testing workflows.
...
`$B state save/load` ships in v0.12.1.0. V1 saves cookies + URLs only (not localStorage, which breaks on load-before-navigate). Files at `.nstack/browse-states/{name}.json` with 0o600 permissions. Load replaces session (closes all pages first). Name sanitized to `[a-zA-Z0-9_-]`.
...
`$B connect` launches Playwright's bundled Chromium in headed mode with the nstack Chrome extension auto-loaded. `$B handoff` now produces the same result (extension + side panel). Sidebar chat gated behind `--chat` flag.
...
**What:** Publish the nstack browse Chrome extension to Chrome Web Store for easier install.
...
**What:** Append structured JSON entry to `.nstack/ship-log.json` at end of every /ship run (version, date, branch, PR URL, review findings, Greptile stats, todos completed, test results).
...
**Effort:** M
**Priority:** P2
**Depends on:** /setup-nstack-upload
...
### /setup-nstack-upload skill (S3 bucket)
...
### nstack-upload helper

**What:** `browse/bin/nstack-upload` — upload file to S3, return public URL.
...
**Context:** Reads `~/.nstack-dev/evals/*.json`. ~200 lines HTML + chart.js via Bun HTTP server.
...
**What:** `nstack-open-url` helper script — detect platform, use `open` (macOS) or `xdg-open` (Linux).

**Why:** The first-time Completeness Principle intro uses macOS `open` to launch the essay. If nstack ever supports Linux, this silently fails.
...
**Why:** Cross-team design discovery at scale. Local `~/.nstack/projects/$SLUG/` keyword-grep discovery works for same-machine users now, but Supabase sync makes it work across the whole team. Duplicate ideas surface, everyone sees what's been explored.

**Context:** /office-hours writes design docs to `~/.nstack/projects/$SLUG/`. The team store already syncs test plans, retro snapshots, QA reports. Design docs follow the same pattern — just add a sync adapter.
...
**What:** Create a placeholder resolver in gen-skill-docs.ts encoding the nstack voice guide (friendly, user-forward, lead with benefits). Inject into /ship Step 5, /document-release Step 5, and reference from CLAUDE.md.
...
`bin/nstack-diff-scope` shipped — categorizes diff into SCOPE_FRONTEND, SCOPE_BACKEND, SCOPE_PROMPTS, SCOPE_TESTS, SCOPE_DOCS, SCOPE_CONFIG. Used by design-review-lite to skip when no frontend files changed. Dashboard integration for conditional row display is a follow-up.
...
**Effort:** S
**Priority:** P3
**Depends on:** nstack-diff-scope (shipped)
...
**What:** A Codex-native skill (`.agents/skills/nstack-claude/SKILL.md`) that runs `claude -p` to get an independent second opinion from Claude — the reverse of what `/codex` does today from Claude Code.
...
**Context:** The `/codex` skill template (`codex/SKILL.md.tmpl`) shows the pattern — it wraps `codex exec` with JSONL parsing, timeout handling, and structured output. The reverse skill would wrap `claude -p` with similar infrastructure. Would be generated into `.agents/skills/nstack-claude/` by `gen-skill-docs --host codex`.
...
**What:** Track how often Claude chooses the complete option vs shortcut across nstack sessions. Aggregate into a dashboard showing completeness trend over time.
...
5. Debug session persistence (~/.nstack/investigate-sessions/ — save investigation for reuse)
...
**What:** Add ~10 lines of prose to the preamble telling the agent to re-read nstack artifacts (CEO plans, design reviews, eng reviews, checkpoints) after compaction or context degradation.

**Why:** nstack skills produce valuable artifacts stored at `~/.nstack/projects/$SLUG/`. When Claude's auto-compaction fires, it preserves a generic summary but doesn't know these artifacts exist. The plans and reviews that shaped the current work silently vanish from context, even though they're still on disk. This is the thing nobody else in the Claude Code ecosystem is solving, because nobody else has nstack's artifact architecture.

**Context:** Inspired by Anthropic's `claude-progress.txt` pattern for long-running agents. Also informed by claude-mem's "progressive disclosure" approach. See `docs/designs/SESSION_INTELLIGENCE.md` for the broader vision. CEO plan: `~/.nstack/projects/garrytan-nstack/ceo-plans/2026-03-31-session-intelligence-layer.md`.
...
**What:** Append one-line JSONL entry to `~/.nstack/projects/$SLUG/timeline.jsonl` after every skill run (timestamp, skill, branch, outcome). `/retro` renders the timeline.
...
**What:** When a new nstack session starts on a branch with recent checkpoints or plans, the preamble prints a one-line summary: "Last session: implemented JWT auth, 3/5 tasks done." Agent knows where you left off before reading any files.
...
**What:** Manual skill to snapshot current working state: what's being done and why, files being edited, decisions made (and rationale), what's done vs. remaining, critical types/signatures. Saved to `~/.nstack/projects/$SLUG/checkpoints/<timestamp>.md`.
...
**What:** Write `docs/designs/SESSION_INTELLIGENCE.md` describing the architectural vision: nstack as the persistent brain that survives Claude's ephemeral context. Every skill writes to `~/.nstack/projects/$SLUG/`, preamble re-reads, `/retro` rolls up.
...
**What:** Skill that runs type-check, lint, test suite, and dead code scan, then reports a composite 0-10 health score with breakdown by category. Tracks over time in `~/.nstack/health/<project-slug>/` for trend detection. Optionally integrates CodeScene MCP for deeper complexity/cohesion/coupling analysis.
...
**What:** Expose nstack's browse binary and key workflows as an MCP server that Factory Droid connects to natively. Factory users would run /mcp, add the nstack server, and get browse, QA, and review capabilities as Factory tools.

**Why:** Factory already supports 40+ MCP servers in its registry. Getting nstack's browse binary listed there is a distribution play. Nobody else has a real compiled browser binary as an MCP tool. This is the thing that makes nstack uniquely valuable on Factory Droid.
...
**What:** Factory has "custom droids" (subagents with tool restrictions, model selection, autonomy levels). Could ship `nstack-qa.md` droid configs alongside skills that restrict tools to read-only + execute for safety.

**Why:** Deeper Factory integration. Droid configs give Factory users tighter control over what nstack skills can do.
...
- Config CLI (`bin/nstack-config`), auto-upgrade via `~/.nstack/config.yaml`, 12h cache TTL, exponential snooze backoff (24h→48h→1wk), "never ask again" option, vendored copy sync on upgrade
**Completed:** v0.3.8
