...
- **Show screenshots:** After `$B screenshot`, `$B snapshot -a -o`, or `$B responsive`, always use the view_file tool on the output PNG(s) so the user can see them. Without this, screenshots are invisible.
...
`$B` is the browse binary (resolved from `$_ROOT/.antigravity/skills/gstack/browse/dist/browse` or `~/.gemini/antigravity/skills/gstack/browse/dist/browse`).
