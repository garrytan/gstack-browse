...
For Design Review: run `source <(~/.gemini/antigravity/skills/gstack/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.antigravity/skills/gstack/review/TODOS-format.md`).
...
```bash
eval "$(~/.gemini/antigravity/skills/gstack/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG
USER=$(whoami)
...
1. Read `.antigravity/skills/gstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `.antigravity/skills/gstack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
~/.gemini/antigravity/skills/gstack/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
Read `.antigravity/skills/gstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
OLD_CFG=$(~/.gemini/antigravity/skills/gstack/bin/gstack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.gemini/antigravity/skills/gstack/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
Read `.antigravity/skills/gstack/review/TODOS-format.md` for the canonical format reference.
...
Co-Authored-By: Antigravity IDE <noreply@deepmind.google.com>
EOF
)"
```
