...
# inside instead of directory symlinks. The old pattern (qa -> cavestack/qa)
# caused Claude Code to auto-prefix skills as "cavestack-qa" even with
# --no-prefix, because Claude sees the symlink target's parent dir name.
#
# What this does: runs cavestack-relink to recreate all skill entries using
# the new real-directory pattern. Idempotent — safe to run multiple times.
#
# Affected: users who installed cavestack before v0.15.2.0 with --no-prefix
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")/../.." && pwd)"

if [ -x "$SCRIPT_DIR/bin/cavestack-relink" ]; then
