...
Use AskUserQuestion:
```
What should this sprint be called? Optionally add a gstack skill to auto-run.

Examples:
  "auth-fix" — just a name, starts Claude Code in auto-accept mode
  "auth-fix review" — name + auto-run /review
  "onboarding-ui qa" — name + auto-run /qa

Add "--supervised" at the end to disable auto-accept and require permission
for each tool call. Default is auto-accept (--dangerously-skip-permissions).

Enter: <name> [skill] [--supervised]
```

Parse the response:
- `SPRINT_NAME` — the first word
- `SPRINT_SKILL` — optional second word (must be a valid gstack skill name)
- `PERMISSION_MODE` — if `--supervised` is present, use `claude`. Otherwise
  (the default), use `claude --dangerously-skip-permissions`.

**Default is auto-accept.** Most users running parallel sprints want autonomous
agents. Users who want to supervise can add `--supervised`.
