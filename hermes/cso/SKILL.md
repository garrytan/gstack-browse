---
name: cso
description: Use when chief security officer mode. (Ported from gstack to Hermes)
version: 1.0.0
...
metadata:
  hermes:
    ...
    upstream: https://github.com/garrytan/gstack/blob/main/cso/SKILL.md
---
## When to Use
...
> **STOP.** Before running the scope-dependent audit phases (Phases 2-11) selected by the resolved mode, after the Phase 0 stack detection and Phase 1 attack-surface census, Read `~/.hermes/skills/gstack/cso/sections/audit-phases.md` and execute it
...
For each candidate finding, launch an independent verification sub-task using delegate_task. The verifier has fresh context and cannot see the initial scan's reasoning — only the finding itself and the FP filtering rules.
...
If delegate_task is unavailable, self-verify by re-reading code with a skeptic's eye. Note: "Self-verified — independent sub-task unavailable."
...
```bash
~/.hermes/skills/gstack/# memory tool '{"skill":"cso","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
