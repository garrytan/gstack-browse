---
name: design-review
description: Use when designer's eye qa: finds visual inconsistency, spacing issues, hierarchy problems, ai slop patterns, and slow interactions — then fixes them. (Ported from gstack to Hermes)
version: 1.0.0
...
metadata:
  hermes:
    ...
    upstream: https://github.com/garrytan/gstack/blob/main/design-review/SKILL.md
---
## When to Use
...
2. Take a full-page desktop screenshot: `terminal screenshot "$REPORT_DIR/screenshots/first-impression.png"`
...
terminal js "JSON.stringify([...new Set([...document.querySelectorAll('*')].slice(0,500).map(e => getComputedStyle(e).fontFamily))])"

# Color palette in use
terminal js "JSON.stringify([...new Set([...document.querySelectorAll('*')].slice(0,500).flatMap(e => [getComputedStyle(e).color, getComputedStyle(e).backgroundColor]).filter(c => c !== 'rgba(0, 0, 0, 0)'))])"

# Heading hierarchy
terminal js "JSON.stringify([...document.querySelectorAll('h1,h2,h3,h4,h5,h6')].map(h => ({tag:h.tagName, text:h.textContent.trim().slice(0,50), size:getComputedStyle(h).fontSize, weight:getComputedStyle(h).fontWeight})))"

# Touch target audit (find undersized interactive elements)
terminal js "JSON.stringify([...document.querySelectorAll('a,button,input,[role=button]')].filter(e => {const r=e.getBoundingClientRect(); return r.width>0 && (r.width<44||r.height<44)}).map(e => ({tag:e.tagName, text:(e.textContent||'').trim().slice(0,30), w:Math.round(e.getBoundingClientRect().width), h:Math.round(e.getBoundingClientRect().height)})).slice(0,20))"
...
```bash
terminal goto <url>
terminal snapshot -i -a -o "$REPORT_DIR/screenshots/{page}-annotated.png"
terminal responsive "$REPORT_DIR/screenshots/{page}"
...
- `text-wrap: balance` or `text-pretty` on headings (check via `terminal css <heading> text-wrap`)
...
- `prefers-reduced-motion` respected (check: `terminal js "matchMedia('(prefers-reduced-motion: reduce)').matches"`)
...
```bash
terminal snapshot -i
terminal click @e3           # perform action
terminal snapshot -D          # diff to see what changed
```
...
```
Write to: `~/.hermes/gstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
```
~/.hermes/gstack/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
terminal screenshot "$REPORT_DIR/screenshots/finding-NNN-after.png"
terminal console --errors
terminal snapshot -D
```
...
eval "$(~/.hermes/skills/gstack/# session_search tool 2>/dev/null)" && mkdir -p ~/.hermes/gstack/projects/$SLUG
```
Write a one-line summary to `~/.hermes/gstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
...
```bash
~/.hermes/skills/gstack/# memory tool '{"skill":"design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
