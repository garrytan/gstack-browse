...
description: Use when designer's eye plan review — interactive, like ceo and eng review. (Ported from gstack to Hermes)
version: 1.0.0
...
metadata:
  hermes:
    ...
    upstream: https://github.com/garrytan/gstack/blob/main/plan-design-review/SKILL.md
---
## When to Use
...
eval "$(~/.hermes/skills/gstack/# session_search tool 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "brand"
  ~/.hermes/skills/gstack/bin/gstack-brain-cache get brand --project "$SLUG" 2>/dev/null || printf '_(no brand digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
...
`~/.hermes/gstack/projects/$SLUG/designs/` (user config directory, not project files).
...
- `mkdir -p ~/.hermes/gstack/projects/$SLUG/designs/...`
- `$D generate`, `$D variants`, `$D compare`, `$D iterate`, `$D evolve`, `$D check`
- `open` (fallback for viewing boards when `terminal` is not available)
...
> **STOP.** Before running the 7 design passes, required outputs, and review report (only after Step 0 scope is agreed), Read `~/.hermes/skills/gstack/plan-design-review/sections/review-sections.md` and execute it
