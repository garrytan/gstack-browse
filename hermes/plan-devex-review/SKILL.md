...
description: Use when interactive developer experience plan review. (Ported from gstack to Hermes)
version: 1.0.0
...
metadata:
  hermes:
    ...
    upstream: https://github.com/garrytan/gstack/blob/main/plan-devex-review/SKILL.md
---
## When to Use
...
eval "$(~/.hermes/skills/gstack/# session_search tool 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "developer-persona"
  ~/.hermes/skills/gstack/bin/gstack-brain-cache get developer-persona --project "$SLUG" 2>/dev/null || printf '_(no developer-persona digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
  ...
  printf '\n### %s\n\n' "competitive-intel"
  ~/.hermes/skills/gstack/bin/gstack-brain-cache get competitive-intel --project "$SLUG" 2>/dev/null || printf '_(no competitive-intel digest available yet)_\n'
...
> **STOP.** Before running the 8 DX passes, required outputs, and review report (only after Step 0 investigation is complete), Read `~/.hermes/skills/gstack/plan-devex-review/sections/review-sections.md` and execute it
