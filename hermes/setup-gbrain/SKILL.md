---
name: setup-gbrain
description: Use when set up gbrain for this coding agent: install the cli, initialize a local pglite or supabase brain, register mcp, capture per-remote trust policy. (Ported from gstack to Hermes)
version: 1.0.0
...
metadata:
  hermes:
    ...
    upstream: https://github.com/garrytan/gstack/blob/main/setup-gbrain/SKILL.md
---
## When to Use
...
| persisting the Step 8 `## GBrain Configuration` block to AGENTS.md (and the Search Guidance block after Step 9 passes) | `sections/claude-md-persist.md` |
...
> **STOP.** Before running the Step 1.5 broken-engine remediation — Step 1's detect returned `gbrain_local_status` of `broken-db` or `broken-config` and no shortcut flag was passed, Read `~/.hermes/skills/gstack/setup-gbrain/sections/engine-remediation.md` and execute it
...
```bash
[ -f ~/.hermes/skills/gstack/bin/gstack-code-intelligence ] \
  && bun ~/.hermes/skills/gstack/bin/gstack-code-intelligence select gbrain 2>/dev/null \
  || true
```
...
> **STOP.** Before initializing the brain in Step 4 — run ONLY the procedure for the path picked in Step 2 (Paths 1/2a/2b/3/4 or Switch; also holds the PAT scope disclosure that `--cleanup-orphans` re-uses), Read `~/.hermes/skills/gstack/setup-gbrain/sections/brain-init.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
any gbrain client. The helper creates a `git worktree` of `~/.hermes/gstack/`,
...
or no `~/.hermes/gstack/.git` yet) so the user sees the failure rather than silently
...
> **STOP.** Before running the Step 7.5 transcript & memory ingest gate on Paths 1, 2a, 2b, or 3 (Path 4 skips this section entirely — see the skeleton's skip note), Read `~/.hermes/skills/gstack/setup-gbrain/sections/transcript-gate.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
> **STOP.** Before persisting the Step 8 `## GBrain Configuration` block to AGENTS.md (and the Search Guidance block after Step 9 passes), Read `~/.hermes/skills/gstack/setup-gbrain/sections/claude-md-persist.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
The brain trust policy controls whether gstack auto-pushes `~/.hermes/gstack/`
...
> shared/team brain?
>
> Personal: gstack auto-pushes ~/.hermes/gstack/ artifacts (CEO plans, design
...
[ -f ~/.hermes/gstack/.gbrain-sync-state.json ] && cat ~/.hermes/gstack/.gbrain-sync-state.json || echo "{}"
```
...
`~/.hermes/gstack/transcripts/run-<pid>-<ts>/` and # memory tool / session_search pushes them
...
- **Concurrent-run lock.** At skill start, `mkdir ~/.hermes/gstack/.setup-gbrain.lock.d`
  (atomic). If the mkdir fails, abort with: "Another `/setup-gbrain` instance
  is running. Wait for it, or `rm -rf ~/.hermes/gstack/.setup-gbrain.lock.d` if
