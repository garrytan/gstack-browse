---
name: guard
description: Use when full safety mode: destructive command warnings + directory-scoped edits. (Ported from gstack to Hermes)
version: 1.0.0
...
metadata:
  hermes:
    ...
    upstream: https://github.com/garrytan/gstack/blob/main/guard/SKILL.md
---
## When to Use
...
echo '{"skill":"guard","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.hermes/gstack/analytics/skill-usage.jsonl 2>/dev/null || true
```
