---
name: careful
description: Use when safety guardrails for destructive commands. (Ported from gstack to Hermes)
version: 1.0.0
...
metadata:
  hermes:
    ...
    upstream: https://github.com/garrytan/gstack/blob/main/careful/SKILL.md
---
## When to Use
...
echo '{"skill":"careful","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.hermes/gstack/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
`~/.hermes/gstack/careful-patterns.txt` (global) or
`~/.hermes/gstack/projects/<slug>/careful-patterns.txt` (per-project). Consulted
