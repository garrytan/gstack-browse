...
description: Use when ceo/founder-mode plan review. (Ported from gstack to Hermes)
version: 1.0.0
...
metadata:
  hermes:
    ...
    upstream: https://github.com/garrytan/gstack/blob/main/plan-ceo-review/SKILL.md
---
## When to Use
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t ~/.hermes/gstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```bash
eval "$(~/.hermes/skills/gstack/# session_search tool 2>/dev/null)" && mkdir -p ~/.hermes/gstack/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.hermes/gstack/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.hermes/gstack/projects/$SLUG/ceo-plans/{old-plan}.md ~/.hermes/gstack/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.hermes/gstack/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
Use delegate_task to dispatch an independent reviewer. The reviewer has fresh context
...
1. Fix each issue in the document on disk (use patch tool)
...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.hermes/gstack/analytics/spec-review.jsonl 2>/dev/null || true
```
...
> **STOP.** Before running the 11-section deep review, required outputs, and review report (only after Step 0 scope and mode are agreed), Read `~/.hermes/skills/gstack/plan-ceo-review/sections/review-sections.md` and execute it
