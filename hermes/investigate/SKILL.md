---
name: investigate
description: Use when systematic debugging with root cause investigation. (Ported from gstack to Hermes)
version: 1.0.0
...
metadata:
  hermes:
    ...
    upstream: https://github.com/garrytan/gstack/blob/main/investigate/SKILL.md
---
## When to Use
...
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.hermes/skills/gstack/bin/gstack-learnings-search --limit 10 --query "debug investigation root cause hypothesis bug fix" --cross-project 2>/dev/null || true
else
  ~/.hermes/skills/gstack/bin/gstack-learnings-search --limit 10 --query "debug investigation root cause hypothesis bug fix" 2>/dev/null || true
fi
```
...
_FREEZE_SCRIPT="$HOME/.hermes/skills/gstack/freeze/bin/check-freeze.sh"
...
```bash
~/.hermes/skills/gstack/# memory tool '{"skill":"investigate","type":"investigation","key":"ROOT_CAUSE_KEY","insight":"ROOT_CAUSE_SUMMARY","confidence":9,"source":"observed","files":["affected/file1.ts","affected/file2.ts"]}'
```
...
```bash
~/.hermes/skills/gstack/# memory tool '{"skill":"investigate","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
