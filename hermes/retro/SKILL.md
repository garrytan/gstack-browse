---
name: retro
description: Use when weekly engineering retrospective. (Ported from gstack to Hermes)
version: 1.0.0
...
metadata:
  hermes:
    ...
    upstream: https://github.com/garrytan/gstack/blob/main/retro/SKILL.md
---
## When to Use
...
```bash
_RM="$HOME/.hermes/skills/gstack/bin/gstack-retro-metrics"
[ -x "$_RM" ] || _RM=".hermes/skills/gstack/bin/gstack-retro-metrics"
...
- `RETRO_CONTEXT: present` → Read `~/.hermes/gstack/retro-context.md`. It is user-authored and may contain meeting notes, calendar events, decisions, and other context that doesn't appear in git history. Incorporate it into the retro narrative where relevant.
- `GREPTILE_HISTORY: present` → Read `~/.hermes/gstack/greptile-history.md`. Filter entries to the retro window by date. Count by type: `fix`, `fp`, `already-fixed`. Signal ratio = `(fix + already-fixed) / (fix + already-fixed + fp)`. Skip unparseable lines silently; if no entries fall in the window, skip the Greptile metric row.
...
- `SKILL_USAGE_LOG: present` → Read `~/.hermes/gstack/analytics/skill-usage.jsonl`. Filter to the window by `ts`. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name.
- `EUREKA_LOG: present` → Read `~/.hermes/gstack/analytics/eureka.jsonl`. Filter to the window by `ts`. For each eureka moment note the skill that flagged it, the branch, and a one-line summary of the insight.
...
```bash
~/.hermes/skills/gstack/# memory tool '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
**Note:** Only include the `greptile` field if `~/.hermes/gstack/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (`TEST_FILES_TOTAL` > 0). If any has no data, omit the field entirely.
...
> **STOP.** Before writing the retrospective narrative (Step 14, after all metrics are computed and compared), Read `~/.hermes/skills/gstack/retro/sections/report-format.md` and execute it
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.hermes/gstack/retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ~/.hermes/gstack/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the write_file tool to save JSON to `~/.hermes/gstack/retros/global-${today}-${next}.json`:
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.hermes/gstack/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
