...
### 4.0 Write files via Bash (plan mode compatible)

**CRITICAL:** The report and baseline MUST be written to `~/.gstack/projects/$SLUG/audits/`. If the session is in plan mode, the Write tool may be restricted to the plan file. To avoid this, **always use Bash with heredoc** to write the report and baseline files — Bash is not restricted by plan mode. Example:

```bash
cat > ~/.gstack/projects/$SLUG/audits/{datetime}-audit.md << 'AUDIT_EOF'
{report content here}
AUDIT_EOF
```

Do NOT use the Write tool for the report or baseline. Do NOT write the report to a plan file. Do NOT write the report content into the plan file "temporarily" — it will trigger "Ready to code?" which is wrong for an audit.
...
**Step 1: Apply mechanical fixes NOW** (before writing any plan file).
Exit the read-only audit constraint. Apply each mechanical fix via Edit/Bash, commit atomically with messages referencing the finding ID (e.g., `fix: add WebSocket auth check on /ws/ctrl (audit S-1)`). These are done — no plan needed.

**Step 2: Write a plan for ONLY the substantive fixes.**
The plan file should contain ONLY the substantive findings that need review. Do NOT include the mechanical fixes — they're already committed. At the very top of the plan file, include a review recommendation banner:

**If findings involve scope/product decisions** (new abstractions, architecture redesign, changing public interfaces, adding new packages, significant pattern changes across the codebase):
```
> **Next step:** Run `/plan-ceo-review` on this plan first (scope decisions), then `/plan-eng-review` (architecture review).
> These fixes involve design decisions that benefit from structured review before implementation.
```

**If findings are implementation-level** (write tests, add error handling, fix resource leaks, add timeouts — things with clear solutions but touching multiple files):
```
> **Next step:** Run `/plan-eng-review` on this plan before implementing.
> The substantive fixes below touch multiple files and benefit from architectural review.
```

When the user sees "Ready to code?" it will be for the substantive fixes only. The banner tells them to run `/plan-eng-review` (or `/plan-ceo-review`) instead of accepting directly. If they accept anyway, the plan contains only the substantive work.

**Step 3: Tell the user what happened.**
Before the plan file triggers "Ready to code?", output a brief message: "Applied N mechanical fixes (committed). The plan contains only the substantive fixes — run `/plan-eng-review` before implementing."
