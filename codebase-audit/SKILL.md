...
### 4.7 Presentation preference

After printing the conversation summary, use AskUserQuestion to ask: "Full findings are in the report file. Would you like me to display all findings inline here, or is the top-5 summary sufficient?"

If the user wants inline findings, print all findings grouped by severity.
...
7. AskUserQuestion fires in two places: (1) Phase 1 if >50K LOC, to scope the audit; (2) end of Phase 4, for presentation preference. Do not use it elsewhere.
...
13. Do NOT write to plan files or enter plan mode. This skill produces a report, not an implementation plan. Write only to `~/.gstack/projects/$SLUG/audits/`. If the session is in plan mode, exit plan mode after writing the report — the audit is complete, not a proposal for future work.
