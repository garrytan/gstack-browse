...
3. For the remaining substantive findings, write a plan summarizing what needs to be done. **At the very top of the plan file**, before the Context section, include a review recommendation banner. Choose the appropriate one based on the nature of the findings:
...
This banner ensures the user sees the review recommendation in the "Ready to code?" preview, even though plan mode takes over the conversation flow after the plan is written.

4. End the skill. The user sees the plan with the review recommendation and can either approve it directly or run the appropriate review first.
