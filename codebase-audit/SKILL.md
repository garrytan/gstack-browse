...
**After writing the plan**, output a brief message classifying what's in it: "Plan written: {M} mechanical fixes (Part 1) + {S} substantive fixes (Part 2, recommend `/plan-eng-review` first)."
...
7. AskUserQuestion fires in one place: Phase 1 if >50K LOC, to scope the audit. The fix plan (Phase 4.7) uses the plan file for next steps — "Ready to code?" is the user's decision point. Do not use AskUserQuestion elsewhere during the audit.
