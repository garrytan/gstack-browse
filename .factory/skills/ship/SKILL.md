...
```bash
_UPD=$(~/.factory/skills/gstack/bin/gstack-update-check 2>/dev/null || .factory/skills/gstack/bin/gstack-update-check 2>/dev/null || true)
[ -n "$_UPD" ] && echo "$_UPD" || true
mkdir -p ~/.gstack/sessions
touch ~/.gstack/sessions/"$PPID"
_SESSIONS=$(find ~/.gstack/sessions -mmin -120 -type f 2>/dev/null | wc -l | tr -d ' ')
find ~/.gstack/sessions -mmin +120 -type f -delete 2>/dev/null || true
_CONTRIB=$(~/.factory/skills/gstack/bin/gstack-config get gstack_contributor 2>/dev/null || true)
_PROACTIVE=$(~/.factory/skills/gstack/bin/gstack-config get proactive 2>/dev/null || echo "true")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
echo "PROACTIVE: $_PROACTIVE"
_LAKE_SEEN=$([ -f ~/.gstack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(~/.factory/skills/gstack/bin/gstack-config get telemetry 2>/dev/null || true)
...
[ -n "$_PF" ] && ~/.factory/skills/gstack/bin/gstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
```

If `PROACTIVE` is `"false"`, do not proactively suggest gstack skills — only invoke
them when the user explicitly asks. The user opted out of proactive suggestions.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.factory/skills/gstack/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If `JUST_UPGRADED <from> <to>`: tell user "Running gstack v{to} (just updated!)" and continue.
...
If A: run `~/.factory/skills/gstack/bin/gstack-config set telemetry community`
...
If B→A: run `~/.factory/skills/gstack/bin/gstack-config set telemetry anonymous`
If B→B: run `~/.factory/skills/gstack/bin/gstack-config set telemetry off`
...
~/.factory/skills/gstack/bin/gstack-telemetry-log \
...
```bash
~/.factory/skills/gstack/bin/gstack-review-read
```
...
1. **Check for a prior override on this branch:**
   ```bash
   source <(~/.factory/skills/gstack/bin/gstack-slug 2>/dev/null)
...
2. **If no override exists,** use AskUserQuestion:
   ...
   - For Design Review: run `source <(~/.factory/skills/gstack/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
```bash
source <(~/.factory/skills/gstack/bin/gstack-diff-scope <base> 2>/dev/null)
```
...
```bash
~/.factory/skills/gstack/bin/gstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
CODEX_REVIEWS_CFG=$(~/.factory/skills/gstack/bin/gstack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.factory/skills/gstack/bin/gstack-config set codex_reviews enabled
```
...
```bash
~/.factory/skills/gstack/bin/gstack-config set codex_reviews disabled
```
...
```bash
~/.factory/skills/gstack/bin/gstack-review-log '{"skill":"codex-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
