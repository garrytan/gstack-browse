...
**Design doc check:**
```bash
SLUG=$(~/.factory/skills/gstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
...
```bash
source <(~/.factory/skills/gstack/bin/gstack-slug 2>/dev/null) && mkdir -p ~/.gstack/projects/$SLUG/ceo-plans
```
...
Use the Task tool to dispatch a subagent with subagent_type: gstack-spec-reviewer. The reviewer has fresh context
...
```bash
~/.factory/skills/gstack/bin/gstack-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","commit":"COMMIT"}'
```
