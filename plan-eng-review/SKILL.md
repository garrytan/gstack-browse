...
DESIGN=$(ls -t "$PROJECT_DATA_DIR/designs/"*-$BRANCH-design-*.md 2>/dev/null || ls -t ~/.gstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t "$PROJECT_DATA_DIR/designs/"*-design-*.md 2>/dev/null || ls -t ~/.gstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
Write to `.gstack/plans/{user}-{branch}-eng-review-test-plan-{datetime}.md`:
