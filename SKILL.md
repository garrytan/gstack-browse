...
description: |
  ...
  gstack also includes development workflow skills and may proactively suggest them
  when the user is brainstorming, reviewing plans, debugging, testing, auditing
  design, shipping, updating docs, running retros, wanting a second opinion,
  upgrading gstack, or asking for extra safety around destructive commands. If the
  user says "stop suggesting things", "I don't need suggestions", or similar, stop
  suggesting for the rest of the session and run `gstack-config set proactive false`.
  If they later say "be proactive again" or "turn on suggestions", run
...
- **Show screenshots:** After `$B screenshot`, `$B snapshot -a -o`, or `$B responsive`, always use the view_image tool on the output PNG(s) so the user can see them. Without this, screenshots are invisible.
