...
If only a slug is given, use the `latest` symlink if available, otherwise fall back to
the most recent timestamp directory:

```bash
if [ -L "research/results/<slug>/latest" ]; then
  echo "research/results/<slug>/latest"
else
  _LATEST=$(ls -t research/results/<slug>/ | head -1)
  echo "research/results/<slug>/$_LATEST"
fi
```
