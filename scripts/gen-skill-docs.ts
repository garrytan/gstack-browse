...
source <(${ctx.paths.sharedBinDir}/gstack-repo-mode 2>/dev/null) || true
...
**Persist the result:**
\`\`\`bash
${ctx.paths.sharedBinDir}/gstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
function extractHookSafetyProse(tmplContent: string): string | null {
	...
	while ((matcher = matcherRegex.exec(tmplContent)) !== null) {
		if (!matchers.includes(matcher[1])) matchers.push(matcher[1]);
	}
