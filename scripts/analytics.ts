#!/usr/bin/env bun
/**
 * analytics — CLI for viewing cavestack skill usage statistics.
 *
 * Reads ~/.cavestack/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.cavestack', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('cavestack skill usage analytics');
