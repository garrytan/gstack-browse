...
Ask these questions **ONE AT A TIME** by asking the user directly. Push on each one until the answer is specific, evidence-based, and uncomfortable. Comfort means the founder hasn't gone deep enough.
...
Ask these **ONE AT A TIME** by asking the user directly. The goal is to brainstorm and sharpen the idea, not interrogate.
...
- Ask by asking the user directly: "Should we build on this prior design or start fresh?"
...
Use ask the user directly to confirm. If the user disagrees with a premise, revise understanding and loop back.
...
Present by asking the user directly. Do NOT proceed without user approval of the approach.
...
Present the design doc to the user by asking the user directly:
...
Then ask the user directly: "Would you consider applying to Y Combinator?"
...
- **Questions ONE AT A TIME.** Never batch multiple questions into one ask the user directly.
