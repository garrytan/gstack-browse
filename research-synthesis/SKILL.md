...
description: |
  ...
  "background research", "evidence synthesis", "systematic review". (gstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"research-synthesis","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"research-synthesis","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
R=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/gstack/research/dist/research-tools" ] && R="$_ROOT/.claude/skills/gstack/research/dist/research-tools"
[ -z "$R" ] && [ -x ~/.claude/skills/gstack/research/dist/research-tools ] && R=~/.claude/skills/gstack/research/dist/research-tools
if [ -n "$R" ] && [ -x "$R" ]; then
...
Run: `cd <gstack-dir> && bun build --compile research/src/cli.ts --outfile research/dist/research-tools && chmod +x research/dist/research-tools`
