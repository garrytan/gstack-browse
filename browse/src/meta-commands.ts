...
const SAFE_DIRECTORIES = getSafeDirectories();
...
export async function handleMetaCommand(
  ...
  switch (command) {
    ...
    case 'pdf': {
      const page = bm.getPage();
      const pdfPath = args[0] || path.join(TEMP_DIR, 'browse-page.pdf');
    ...
    case 'responsive': {
      const page = bm.getPage();
      const prefix = args[0] || path.join(TEMP_DIR, 'browse-responsive');
