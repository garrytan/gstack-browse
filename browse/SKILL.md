...
After `$B screenshot`, `$B snapshot -a -o`, or `$B responsive`, always use the view_image tool on the output PNG(s) so the user can see them. Without this, screenshots are invisible.
...
# 2. Tell the user what happened (by asking the user directly)
