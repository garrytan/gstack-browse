# cavestack x OpenClaw Integration

cavestack integrates with OpenClaw as a methodology source, not a ported codebase.
OpenClaw's ACP runtime spawns Claude Code sessions natively. cavestack provides the
...
```
  ...
  calendar, memory, EA                     methodology + planning
       ...
       │   investigate, retro                   │
       │                                        ├── Generates cavestack-lite
       ├── sessions_spawn(runtime: "acp")       │   (planning discipline)
       │       │                                │
       │       └── Claude Code                  ├── Generates cavestack-full
       │           └── cavestack installed at      │   (complete pipeline)
       │               ~/.claude/skills/cavestack  │
...
OpenClaw decides at spawn time which tier of cavestack support to use:

| Tier | When | Prompt prefix |
|------|------|---------------|
| **Simple** | One-file edits, typos, config changes | No cavestack context injected |
| **Medium** | Multi-file features, refactors | cavestack-lite CLAUDE.md appended |
| **Heavy** | Specific cavestack skill needed | "Load cavestack. Run /X" |
| **Full** | Complete features, objectives, projects | cavestack-full pipeline appended |
| **Plan** | "Help me plan a Claude Code project" | cavestack-plan pipeline appended |
...
The complete ready-to-paste section lives in `openclaw/agents-cavestack-section.md`.
Copy it into your OpenClaw AGENTS.md.

Key behavioral rules (these go ABOVE the dispatch tiers):

1. **Always spawn, never redirect.** When the user asks to use ANY cavestack skill,
...
cavestack-lite/full as a new section. Do not replace the repo's existing instructions.

## What cavestack generates for OpenClaw

All artifacts live in the `openclaw/` directory and are generated by
`bun run gen:skill-docs --host openclaw`:

### cavestack-lite (Medium tier)
`openclaw/cavestack-lite-CLAUDE.md` — ~15 lines of planning discipline:
...
### cavestack-full (Full tier)
`openclaw/cavestack-full-CLAUDE.md` — chains existing cavestack skills:
...
### cavestack-plan (Plan tier)
`openclaw/cavestack-plan-CLAUDE.md` — full review gauntlet, no implementation:
...
- `cavestack-openclaw-office-hours` — Product interrogation (6 forcing questions)
- `cavestack-openclaw-ceo-review` — Strategic challenge (10-section review, 4 modes)
- `cavestack-openclaw-investigate` — Operational debugging (4-phase methodology)
- `cavestack-openclaw-retro` — Operational retrospective (weekly review)

Source lives in `openclaw/skills/` in the cavestack repo. These are hand-crafted
adaptations of the cavestack methodology for OpenClaw's conversational context.
No cavestack infrastructure (no browse, no telemetry, no preamble).

## Spawned session detection

When Claude Code runs inside a session spawned by OpenClaw, the `OPENCLAW_SESSION`
environment variable should be set. cavestack detects this and adjusts:
...
For OpenClaw users: tell your OpenClaw agent "install cavestack for openclaw."

The agent should:
1. Install cavestack-lite CLAUDE.md into its coding session templates
2. Install the 4 native methodology skills
3. Add dispatch routing to AGENTS.md
4. Verify with a test spawn

For cavestack developers: `./setup --host openclaw` outputs this documentation.
...
- No SOUL.md from cavestack (OpenClaw has its own)
