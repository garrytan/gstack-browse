...
(Gemini, Codex, Factory, Kiro, OpenCode, Slate, Cursor) is defined as a typed
TypeScript config object. Adding a new host means creating one file and
...
```
hosts/
├── gemini.ts        # Primary host (Gemini CLI)
...
- What host-specific references to rewrite (paths, tool names)
...
const myhost: HostConfig = {
  ...
  hostSubdir: '.myhost',
  usesEnvVars: true,           // true for most hosts (uses $GSTACK_ROOT env vars)
  ...
  generation: {
    ...
    skipSkills: ['codex'],     // codex skill is not portable to most hosts
  },

  pathRewrites: [
    { from: '~/.gemini/extensions/gstack', to: '~/.myhost/skills/gstack' },
    { from: '.gemini/extensions/gstack', to: '.myhost/skills/gstack' },
    { from: '.gemini/extensions', to: '.myhost/skills' },
  ],
...
export const ALL_HOST_CONFIGS: HostConfig[] = [
  codex, factory, kiro, opencode, slate, cursor, gemini, myhost
];

// Add to re-exports:
export { codex, factory, kiro, opencode, slate, cursor, gemini, myhost };
```
...
# Verify output exists and has no .gemini/extensions leakage
ls .myhost/skills/gstack-*/SKILL.md
grep -r ".gemini/extensions" .myhost/skills/ | head -5
# (should be empty)
...
different tool semantics), use the adapter pattern. Create a module in
`scripts/host-adapters/` and reference it via the `adapter` config field.
