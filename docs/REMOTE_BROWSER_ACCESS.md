# Remote Browser Access — How to Pair With a CaveStack Browser

A CaveStack Browser server can be shared with any AI agent that can make HTTP requests.
...
─────────────                         ────────────
CaveStack Browser Server                 Any AI agent
...
```bash
$B pair-agent --local openclaw    # writes to ~/.openclaw/skills/cavestack/browse-remote.json
$B pair-agent --local codex       # writes to ~/.codex/skills/cavestack/browse-remote.json
$B pair-agent --local cursor      # writes to ~/.cursor/skills/cavestack/browse-remote.json
```
...
3. Save it: `echo 'NGROK_AUTHTOKEN=your_token' > ~/.cavestack/ngrok.env`
4. Optionally claim a stable domain: `echo 'NGROK_DOMAIN=your-name.ngrok-free.dev' >> ~/.cavestack/ngrok.env`
