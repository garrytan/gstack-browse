# Design: cavestack Visual Design Generation (`design` binary)
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.cavestack/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/cavestack-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/cavestack-mockup-{timestamp}.png`
...
```
Note: Codex runtime setup (`setup` script) must also export `CAVESTACK_DESIGN` env var, similar to how `CAVESTACK_BROWSE` is set.
...
| `scripts/resolvers/preamble.ts` | Add `CAVESTACK_DESIGN` env var export for Codex host |
...
1. Read `~/.cavestack/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.cavestack/openai.json` with 0600 permissions
...
- Symlinked via existing `~/.claude/skills/cavestack/` install path
...
- Auth module (read ~/.cavestack/openai.json, fallback to env var, guided setup flow)
...
- Add CAVESTACK_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
