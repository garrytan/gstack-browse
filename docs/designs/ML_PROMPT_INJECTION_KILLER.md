...
**Branch:** JerkyJesse/extension-prompt-injection-defense
**Date:** 2026-03-28
**CEO Plan:** ~/.cavestack/projects/JerkyJesse-cavestack/ceo-plans/2026-03-28-sidebar-prompt-injection-defense.md

## The Problem

The cavestack Chrome extension sidebar gives Claude bash access to control the browser.
...
```typescript
// Public API -- any cavestack component can call these
...
```json
// ~/.cavestack/security/attempts.jsonl
{
...
```
AskUserQuestion:
  "cavestack just blocked a prompt injection attempt from {domain}. These detections
   are rare and valuable for improving defenses for all cavestack users. Can we
...
2. Cache to ~/.cavestack/models/browsesafe-bench/ (not re-downloaded in CI)
...
- cavestack becomes the only open source tool with native-speed prompt injection detection.
...
**Limitation:** macOS-only (Linux would need OpenBLAS FFI). But cavestack already
...
- [ ] Implement `loadModel()` with download-on-first-use to ~/.cavestack/models/
