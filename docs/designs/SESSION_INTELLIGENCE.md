...
cavestack already produces valuable artifacts that survive on disk: CEO plans,
...
survive compaction. Because nobody else has cavestack's artifact architecture.

## The Insight

cavestack already writes structured artifacts to `~/.cavestack/projects/$SLUG/`:
- CEO plans: `ceo-plans/`
...
the agent checks `~/.cavestack/projects/$SLUG/` for recent plans, reviews, and
...
Plan: ~/.cavestack/projects/$SLUG/checkpoints/latest.md"
...
- Not a replacement for Claude's built-in compaction (that handles session
  state; we handle cavestack artifacts)
