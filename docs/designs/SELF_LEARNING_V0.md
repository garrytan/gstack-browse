# Design: CaveStack Self-Learning Infrastructure
...
CaveStack runs 30+ skills across sessions but learns nothing between them. A /review
...
Structured, typed, confidence-scored learnings that every cavestack skill can read and
write. The goal: after 20 sessions on the same codebase, cavestack knows every
...
YC founders building with AI. The people who run cavestack on real codebases 20+ times
...
| **CaveStack** | **Per-project JSONL** | **Cross-session, cross-skill** | **Typed, scored, decaying** |

---

## State Systems

cavestack has four distinct persistence layers. They share storage patterns
(JSONL in `~/.cavestack/projects/$SLUG/`) but serve different purposes:
...
### Release 1: "CaveStack Learns" (v0.13-0.14) — SHIPPED

**Headline:** Every session makes the next one smarter.

What shipped:
- Learnings persistence at `~/.cavestack/projects/{slug}/learnings.jsonl`
...
- E1: Adaptive specialist gating, auto-skip specialists with 0-finding track record.
  Store per-project hit rates via cavestack-learnings-log. User can force with --security etc.
...
- E5: Cross-review finding dedup, read cavestack-review-read for prior review entries.
  Suppress findings matching a prior user-skipped finding.
- E7: Specialist performance tracking, log per-specialist metrics via cavestack-review-log.
...
- Session timeline: every skill auto-logs start/complete events to
  `~/.cavestack/projects/$SLUG/timeline.jsonl`. Local-only, never sent anywhere,
...
- Predictive skill suggestion: if your last 3 sessions follow a pattern
  (review, ship, review), cavestack suggests what you probably want next.
...
- Timeline binaries: `bin/cavestack-timeline-log` and `bin/cavestack-timeline-read`.
...
**Headline:** CaveStack respects your time without compromising your safety.
...
The self-learning roadmap was inspired by ideas from the [Compound Engineering](https://github.com/nicobailon/compound-engineering) project by Nico Bailon. Their exploration of learnings persistence, parallel review agents, and autonomous pipelines catalyzed the design of CaveStack's approach. We adapted every concept to fit CaveStack's template system, voice, and architecture rather than porting directly.
