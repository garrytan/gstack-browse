...
This document covers the command reference and internals of cavestack's headless browser.
...
cavestack's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.cavestack/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
The window has a subtle green shimmer line at the top edge and a floating "cavestack" pill in the bottom-right corner so you always know which Chrome window is being controlled.
...
# Click the cavestack icon in toolbar → Open Side Panel
```
...
4. **Navigate to the extension folder:** Press **Cmd+Shift+G** in the file picker to open "Go to folder", then paste one of these paths:
   - Global install: `~/.claude/skills/cavestack/extension`
   - Dev/source: `<cavestack-repo>/extension`
...
5. **Pin it:** Click the puzzle piece icon (Extensions) in the toolbar → pin "cavestack browse"
6. **Set the port:** Click the cavestack icon → enter the port from `$B status` or `.cavestack/browse.json`
7. **Open Side Panel:** Click the cavestack icon → "Open Side Panel"
...
| **Connection pill** | Small "cavestack" pill in the bottom-right corner of every page when connected |
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.cavestack/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.cavestack/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.cavestack/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.cavestack/browse.json` | Path to state file (CLI passes to server) |
...
The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. cavestack burns zero.
...
cavestack skips all of this. Compiled binary. Plain text in, plain text out. No protocol. No schema. No connection management.
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.cavestack/browse.json`, sends HTTP to the server, prints response. |
...
1. Push your branch
2. Pull in the skill directory: `cd ~/.claude/skills/cavestack && git pull`
3. Rebuild: `cd ~/.claude/skills/cavestack && bun run build`

Or copy the binary directly: `cp browse/dist/browse ~/.claude/skills/cavestack/browse/dist/browse`
