...
2. Read `.gstack/findings.md` if it exists
3. Check for ANY finding with status UNRESOLVED and severity P0 or P1
4. If unresolved P0 findings exist: **BLOCK SHIP** — inform user and list blockers:
   ```
   ...
   ```
5. If unresolved P1 findings exist: **WARN** — ask user for explicit override
6. Read `.gstack/decisions.log` for any ship-blocking decisions
7. Read `.gstack/handoff.md` for recommendations from prior skills

If `.gstack/findings.md` doesn't exist or has no unresolved findings, continue silently.
...
1. Archive `.gstack/session.json` to `.gstack/checkpoints/ship-$(date -u +%Y%m%dT%H%M%SZ).json`
2. Reset `session.json` to idle state (skill=null, phase="idle", turn_count=0, empty arrays)
3. Keep `.gstack/findings.md` and `.gstack/decisions.log` intact (they're the audit trail)
4. Delete `.gstack/handoff.md` (it's been consumed)

If `.gstack/` doesn't exist, skip this step.
