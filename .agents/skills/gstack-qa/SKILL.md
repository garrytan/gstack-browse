...
3. Read `.gstack/handoff.md` if it exists — incorporate prior context (e.g., from /review)
4. Set `session.json`: skill="qa", phase="setup", turn_count=0
...
2. Write it to `.gstack/findings.md` if it's a bug
3. Note the URL, viewport state, and observation in `.gstack/session.json`
...
When a fix is classified, update the finding in `.gstack/findings.md` with:
- Status: RESOLVED (or DEFERRED if reverted)
- Resolution: description of fix
- Commit SHA (if fixed)
Update `.gstack/session.json` accordingly.
...
- Passes → commit: `git commit -m "test(qa): regression test for ISSUE-NNN — {desc}"`
  Update the finding in `.gstack/findings.md` with: `Regression test: {test file and test name}`
...
2. Read `.gstack/session.json` to verify all phases completed
3. Write `.gstack/handoff.md` with the full QA summary, unresolved findings, and recommendations
4. Present the report based on the FILES, not your memory

The final report MUST match what's in `.gstack/findings.md`. If your memory
