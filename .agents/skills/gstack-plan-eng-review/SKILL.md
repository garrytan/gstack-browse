...
### TODOS.md updates
After all review sections are complete, present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `$([ -f .agents/skills/gstack/review/TODOS-format.md ] && echo .agents/skills/gstack/review/TODOS-format.md || echo ~/.codex/skills/gstack/review/TODOS-format.md)`.
...
```bash
$([ -x .agents/skills/gstack/bin/gstack-review-log ] && echo .agents/skills/gstack/bin/gstack-review-log || echo ~/.codex/skills/gstack/bin/gstack-review-log) '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","commit":"COMMIT"}'
```
