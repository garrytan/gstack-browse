...
[ -z "$_AUTO" ] && _AUTO=$("$HOME/.claude/skills/gstack/bin/gstack-config" get auto_upgrade 2>/dev/null || "$_CODEX_HOME/skills/gstack/bin/gstack-config" get auto_upgrade 2>/dev/null || true)
...
```bash
"$HOME/.claude/skills/gstack/bin/gstack-config" set auto_upgrade true 2>/dev/null || \
"$_CODEX_HOME/skills/gstack/bin/gstack-config" set auto_upgrade true
```
...
**If "Never ask again":**
```bash
"$HOME/.claude/skills/gstack/bin/gstack-config" set update_check false 2>/dev/null || \
"$_CODEX_HOME/skills/gstack/bin/gstack-config" set update_check false
```
Tell user: "Update checks disabled. Run `~/.claude/skills/gstack/bin/gstack-config set update_check true` or `~/.codex/skills/gstack/bin/gstack-config set update_check true` to re-enable."
...
elif [ -d "$_CODEX_HOME/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-codex-git"
  INSTALL_DIR="$_CODEX_HOME/skills/gstack"
elif [ -d ".claude/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".claude/skills/gstack"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-codex-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d ".claude/skills/gstack" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR=".claude/skills/gstack"
elif [ -d ".agents/skills/gstack" ]; then
  INSTALL_TYPE="vendored-codex"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d "$HOME/.claude/skills/gstack" ]; then
  INSTALL_TYPE="vendored-global"
  INSTALL_DIR="$HOME/.claude/skills/gstack"
elif [ -d "$_CODEX_HOME/skills/gstack" ]; then
  INSTALL_TYPE="vendored-global-codex"
  INSTALL_DIR="$_CODEX_HOME/skills/gstack"
else
...
**For git installs** (global-git, global-codex-git, local-git, local-codex-git):
```bash
cd "$INSTALL_DIR"
...
**For vendored installs** (vendored, vendored-codex, vendored-global, vendored-global-codex):
```bash
...
if [ -z "$LOCAL_GSTACK" ] && [ -n "$_ROOT" ] && [ -d "$_ROOT/.agents/skills/gstack" ]; then
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit the vendored gstack directory when you're ready."
...
```bash
"$HOME/.claude/skills/gstack/bin/gstack-update-check" --force 2>/dev/null || \
"${CODEX_HOME:-$HOME/.codex}/skills/gstack/bin/gstack-update-check" --force 2>/dev/null || \
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit the vendored gstack directory when you're ready."
