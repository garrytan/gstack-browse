## Coding Tasks (cavestack)

### Rules (non-negotiable)

1. **Always spawn, never redirect.** When user asks to use ANY cavestack skill,
...
→ sessions_spawn(runtime: "acp", prompt: "<cavestack-lite content>\n\n<task>")

**HEAVY:** needs a specific cavestack methodology
→ sessions_spawn(runtime: "acp", prompt: "Load cavestack. Run /qa https://...")
  Skills: /cso, /review, /qa, /ship, /investigate, /design-review, /benchmark, /cavestack-upgrade

**FULL:** build a complete feature, multi-day scope, needs planning + review
→ sessions_spawn(runtime: "acp", prompt: "<cavestack-full content>\n\n<task>")
...
→ sessions_spawn(runtime: "acp", prompt: "<cavestack-plan content>\n\n<task>")
...
- "Upgrade cavestack", "update cavestack" → **HEAVY** with `Run /cavestack-upgrade`
