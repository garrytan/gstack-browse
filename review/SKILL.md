...
If there are ASK items remaining, present them in ONE ask the user directly:
...
If 3 or fewer ASK items, you may use individual ask the user directly calls instead of batching.
...
2. **FALSE POSITIVE comments:** Present each one by asking the user directly:
...
3. **VALID BUT ALREADY FIXED comments:** Reply using the **Already Fixed reply template** from greptile-triage.md — no ask the user directly needed:
...
```bash
~/.codex/skills/gstack-codex/bin/gstack-review-log '{"skill":"codex-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","gate":"GATE"}'
```
