...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || "$HOME/.claude/skills/gstack/browse/bin/remote-slug" 2>/dev/null || "${CODEX_HOME:-$HOME/.codex}/skills/gstack/browse/bin/remote-slug" 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
