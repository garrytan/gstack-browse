...
nstack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/nstack/` (your global install). But when you're developing nstack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone <repo> && cd nstack
...
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where nstack annoyed you)
...
This is the best way to contribute: fix nstack while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ nstack sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on nstack inside the nstack repo

When you're editing nstack skills and want to test them by actually using nstack
...
When E2E tests run, they produce machine-readable artifacts in `~/.nstack-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.nstack-dev/` for post-mortem debugging and trend analysis.
...
nstack generates SKILL.md files for 8 hosts from one set of `.tmpl` templates.
...
|--------|---------------------------|
| Output directory | `{skill}/SKILL.md` vs `.agents/skills/nstack-{skill}/SKILL.md` |
| Frontmatter | Full (name, description, hooks, version) vs minimal (name + description) |
| Paths | `~/.claude/skills/nstack` vs `$NSTACK_ROOT` |
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/nstack`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop nstack.** Symlink your nstack checkout
...
```bash
# In your core project (not the nstack repo)
ln -sfn /path/to/your/nstack-checkout .claude/skills/nstack
```

### Step 2: Run setup to create per-skill symlinks

The `nstack` symlink alone isn't enough. Claude Code discovers skills through
individual top-level directories (`qa/SKILL.md`, `ship/SKILL.md`, etc.), not through
the `nstack/` directory itself. Run `./setup` to create them:
...
Remove the project-local symlink. Claude Code falls back to `~/.claude/skills/nstack/`:
...
- Renamed or moved config keys in `~/.nstack/config.yaml`
- Need to delete orphaned files from a previous version
- Changed the format of `~/.nstack/` state files
...
1. Create `nstack-upgrade/migrations/v{VERSION}.sh` where `{VERSION}` matches
   the VERSION file for the release that needs the fix.
2. Make it executable: `chmod +x nstack-upgrade/migrations/v{VERSION}.sh`
...
During `/nstack-upgrade`, after `./setup` completes (Step 4.75), the upgrade
skill scans `nstack-upgrade/migrations/` and runs every `v*.sh` script whose
