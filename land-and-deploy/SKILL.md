...
version: 1.0.0
description: |
  Land and deploy 工作流. Merges the PR, waits for CI and deploy,
...
- `/land-and-deploy #123 <url>` — specific PR + 验证 URL

## Non-interactive philosophy (like /ship) — with one critical gate

This is a **mostly automated** 工作流. Do NOT ask for confirmation at any step except
...
- Deploy 工作流 failure (offer revert)
...
2. Parse arguments. If the user specified `#NNN`, use that PR number. If a URL was provided, save it for canary 验证 in Step 7.
...
time. That could mean a new 平台, a different 工作流, or updated URLs. I'm going to
do a quick dry run to make sure I still understand how your project deploys."

Then proceed to the FIRST_RUN flow below (步骤 1.5a through 1.5e).
...
and skip manual detection. If no persisted config exists, use the auto-detected 平台
to guide deploy 验证. If nothing is detected, ask the user via AskUserQuestion
in the decision tree below.

If you want to persist deploy settings for future runs, suggest the user run `/setup-deploy`.

Parse the output and record: the detected 平台, production URL, deploy 工作流 (if any),
...
Run whichever commands are relevant based on the detected 平台. Build the results into this table:
...
If 平台 CLI is not installed, note "The {平台} CLI isn't installed on this machine.
I can still deploy through GitHub, but I'll use HTTP health checks instead of the 平台
...
2. **GitHub Actions staging 工作流:** Check for 工作流 files with "staging" in the name or content:
```bash
for f in $(find .github/workflows -maxdepth 1 \( -name '*.yml' -o -name '*.yaml' \) 2>/dev/null); do
  [ -f "$f" ] && grep -qiE "staging" "$f" 2>/dev/null && echo "STAGING_WORKFLOW:$f"
done
...
- A) Run a quick review (~2 min) — I'll scan the diff for 常见 issues like SQL safety, race conditions, and security gaps (Completeness: 7/10)
...
1. **Missing 特性** — commits that add significant functionality not mentioned in the PR
...
new 特性 (new files, new commands, new skills): **WARNING — /document-release
likely not run. CHANGELOG and VERSION not updated despite new 特性.**
...
If the user chooses B: **STOP.** Give specific next 步骤:
...
If `--auto` is 不可用 (repo doesn't have auto-merge enabled), merge directly:
...
Look for runs matching the merge commit SHA. If a deploy 工作流 is found:
- Tell the user: "PR merged. I can see a deploy 工作流 ('{工作流-name}') kicked off automatically. I'll monitor it and let you know when it's done."

If no deploy 工作流 is found after merge:
- Tell the user: "PR merged. I don't see a deploy 工作流 — your project might deploy a different way, or it might be a library/CLI that doesn't have a deploy step. I'll figure out the right 验证 in the next step."

If `MERGE_PATH=auto` and the repo uses merge queues AND a deploy 工作流 exists:
- Tell the user: "PR made it through the merge queue and the deploy 工作流 is running. Monitoring it now."
...
1. If the user provided a production URL as an argument: use it for canary 验证. Also check for deploy 工作流.

2. Check for GitHub Actions deploy 工作流:
```bash
gh run list --branch <base> --limit 5 --json name,status,conclusion,headSha,workflowName
```
Look for 工作流 names containing "deploy", "release", "production", or "cd". If found: poll the deploy 工作流 in Step 6, then run canary.

3. If SCOPE_DOCS is the only scope that's true (no frontend, no backend, no config): skip 验证 entirely. Tell the user: "This was a docs-only change — nothing to deploy or verify. You're all set." Go to Step 9.

4. If no deploy 工作流 detected and no URL provided: use AskUserQuestion once:
   - **Re-ground:** "PR is merged, but I don't see a deploy 工作流 or a production URL for this project. If this is a web app, I can verify the deploy if you give me the URL. If it's a library or CLI tool, there's nothing to verify — we're done."
   - **RECOMMENDATION:** Choose B if this is a library/CLI tool. Choose A if this is a web app.
   - A) Here's the production URL: {let them 类型 it}
...
Use AskUserQuestion:
- **Re-ground:** "I found a staging environment at {staging URL or 工作流}. Since this deploy includes code changes, I can verify everything works on staging first — before it hits production. This is the safest path: if something breaks on staging, production is untouched."
...
Run 步骤 6-7 against the staging target first. Use the staging
URL or staging 工作流 for deploy 验证 and canary checks. After staging passes,
tell the user: "Staging is healthy — your changes are working. Now deploying to production." Then run
步骤 6-7 again against the production target.
...
Run 步骤 6-7 against the staging target. After 验证,
...
Match by the merge commit SHA (captured in Step 4). If multiple matching 工作流, prefer the one whose name matches the deploy 工作流 detected in Step 5.
...
### 常见: Timing and failure handling
...
- **Re-ground:** "The deploy 工作流 failed after the merge. The code is merged but may not be live yet. Here's what I can do:"
...
If timeout (20 min): "The deploy has been running for 20 minutes, which is longer than most deploys take. The site might still be deploying, or something might be stuck." Ask whether to continue waiting or skip 验证.

---

## Step 7: Canary 验证 (conditional depth)

Tell the user: "Deploy is done. Now I'm going to check the live site to make sure everything looks good — loading the page, checking for errors, and measuring 表现."
...
- **Re-ground:** "I found some issues on the live site after the deploy. Here's what I see: {specific issues}. This might be temporary (caches clearing, CDN propagating) or it might be a real 问题."
...
Produce and 展示 the ASCII summary:
...
- If 表现 data was collected: "Want a deeper 表现 analysis? Run `/benchmark <url>`."
...
- **Narrate the journey.** The user should always know: what just happened, what's happening now, and what's about to happen next. No silent gaps between 步骤.
- **Auto-detect everything.** PR number, merge method, deploy strategy, project 类型, merge queues, staging environments. Only ask when information genuinely can't be inferred.
...
- **Single-pass 验证, not continuous monitoring.** `/land-and-deploy` checks once. `/canary` does the extended monitoring loop.
