...
```bash
${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/gstack/bin/gstack-review-read 2>/dev/null
```
...
```bash
eval $(${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/gstack/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
