...
function runLog(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  ...
  try {
    const stdout = execSync(`${BIN}/rstack-learnings-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
function runSearch(args: string = ''): string {
  ...
  try {
    return execSync(`${BIN}/rstack-learnings-search ${args}`, execOpts).trim();
  } catch {
    return '';
  }
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rstack-learn-'));
...
describe('rstack-learnings-search', () => {
...
describe('rstack-learnings-log edge cases', () => {
...
describe('rstack-learnings-search edge cases', () => {
