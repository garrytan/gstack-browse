...
const GEMINI_E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  'gemini-review-findings': ['review/**', '.agents/skills/cavestack-review/**', 'test/helpers/gemini-session-runner.ts'],
};
...
describeGemini('Gemini E2E', () => {
  ...
  testIfSelected('gemini-review-findings', async () => {
    // Run cavestack-review skill via Gemini on worktree (isolated from main working tree)
    const result = await runGeminiSkill({
