...
async function pollChat() {
  ...
  try {
    ...
    if (!resp.ok) {
      console.warn(`[cavestack sidebar] Chat poll failed: ${resp.status} ${resp.statusText}`);
      return;
    }
    ...
    console.error('[cavestack sidebar] Chat poll error:', err.message);
  } finally {
...
async function stopAgent() {
  ...
  try {
    ...
    if (!resp.ok) console.warn(`[cavestack sidebar] Stop agent failed: ${resp.status}`);
  } catch (err) {
    console.error('[cavestack sidebar] Stop agent error:', err.message);
  }
...
async function pollTabs() {
  ...
  try {
    ...
    try {
      ...
      console.debug('[cavestack sidebar] Failed to get active tab URL:', err.message);
    }
    ...
    if (!resp.ok) {
      console.warn(`[cavestack sidebar] Tab poll failed: ${resp.status} ${resp.statusText}`);
      return;
    }
    const data = await resp.json();
    if (!data.tabs) return;
    ...
    console.error('[cavestack sidebar] Tab poll error:', err.message);
  }
}
...
async function switchBrowserTab(tabId) {
  ...
  try {
    ...
    console.error('[cavestack sidebar] Failed to switch browser tab:', err.message);
  }
}
...
document.getElementById('clear-chat').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!resp.ok) console.warn(`[cavestack sidebar] Clear chat failed: ${resp.status}`);
  } catch (err) {
    console.error('[cavestack sidebar] Clear chat error:', err.message);
  }
  // Reset local state
  chatLineCount = 0;
